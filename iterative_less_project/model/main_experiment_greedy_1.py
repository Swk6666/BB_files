...
WARMUP_MIX_RATIO = 1.0  # <--- 【核心修改 1】在这里定义混合训练的比例
...
def _run_command(command: str):
    ...
    try:
        ...
        for output in process.stdout:
            line = output.strip();
            if line: log_lines.append(line); logger.debug(f"[Subprocess]: {line}")
        process.wait()
        retcode = process.poll()
        if retcode != 0:
            logger.error(f"命令执行失败，返回码: {retcode}。"); logger.error("--- 子进程的完整输出日志如下 ---")
            for line in log_lines: logger.error(line)
            logger.error("--- 日志结束 ---"); raise subprocess.CalledProcessError(retcode, command)
        logger.info("命令执行成功。"); return log_lines
    except Exception as e:
        logger.error(f"执行命令时发生未知错误: {e}"); raise
...
def save_df_to_jsonl(df, file_path, columns_to_save, rename_map=None):
    ...
    if rename_map: df_to_save.rename(columns=rename_map, inplace=True)
...
def run_mt5_finetuning(train_file_path, output_dir, num_samples):
    ...
    cmd = (f"python train_imputation_model.py --train_file {train_file_path} --output_dir {output_dir}")
    _run_command(cmd); return output_dir

@torch.no_grad()
def run_entropy_scoring(mt5_model_path, df_to_score, batch_size=32):
    ...
    tokenizer = AutoTokenizer.from_pretrained(mt5_model_path); model.eval()
    results = []
    for i in tqdm(range(0, len(df_to_score), batch_size), desc="Entropy Scoring"):
        batch_df = df_to_score.iloc[i:i+batch_size]; original_indices = batch_df['original_index'].tolist()
        ...
        for step_logits in outputs.scores:
            probs = F.softmax(step_logits, dim=-1); log_probs = F.log_softmax(step_logits, dim=-1)
            step_entropies = -torch.sum(probs * log_probs, dim=-1)
            for j, entropy in enumerate(step_entropies): batch_entropies[j].append(entropy.item())
    ...
    del model, tokenizer; torch.cuda.empty_cache(); return pd.DataFrame(results)

@torch.no_grad()
def run_imputation(mt5_model_path, df_to_impute, batch_size=32):
    ...
    df_imputed = df_to_impute.copy(); df_imputed['imputed_text'] = imputed_texts
    del model, tokenizer; torch.cuda.empty_cache(); return df_imputed
...
def get_less_scores(llama_model_path, val_grad_path, text_df_with_imputed_text, output_dir):
    ...
    if num_samples == 0: return pd.DataFrame(columns=["original_index", "less_score"])
    ...
    train_grad_path = calculate_gradients(model_path=llama_model_path, data_file=temp_less_input_file, num_samples=num_samples, output_path=train_grad_output_path, grad_type="sgd", is_lora=True, base_model_path=config.BASE_MODEL_NAME)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    train_grads = torch.load(train_grad_path, map_location=device); val_grads = torch.load(val_grad_path, map_location=device)
    train_grads_normalized = F.normalize(train_grads, p=2, dim=1); val_grads_normalized = F.normalize(val_grads, p=2, dim=1)
    ...
    torch.cuda.empty_cache(); shutil.rmtree(train_grad_output_path, ignore_errors=True); os.remove(temp_less_input_file)
    return scores_df
...
def main():
    ...
    try:
        ...
        save_df_to_jsonl(warmup_df, warmup_mt5_train_file, columns_to_save=['masked_text']) # train_imputation_model expects 'masked_text'
        mt5_s_warmup_path = run_mt5_finetuning(train_file_path=warmup_mt5_train_file, output_dir=os.path.join(config.ITERATIVE_OUTPUT_DIR, "models", "mt5_s_warmup"), num_samples=len(warmup_df))
        ...
        # --- 【核心修改 2】主动修复 TypeError ---
        val_grad_path = calculate_mmlu_validation_gradients(
            model_path=llama_m_warmup_path,
            output_path=os.path.join(config.ITERATIVE_OUTPUT_DIR, "val_grads"), # <- 修复参数名
            is_lora=True,
        ...
        fixed_less_scores_df = get_less_scores(llama_m_warmup_path, val_grad_path, imputed_candidate_df, os.path.join(config.ITERATIVE_OUTPUT_DIR, "fixed_scores_calculation")).rename(columns={'less_score': 'fixed_less_score'})
        del imputed_candidate_df; torch.cuda.empty_cache()
        ...
        for i in range(1, config.NUM_ITERATIONS + 1):
            phase = i
            num_to_select_total = phase * config.SELECTION_SIZE_PER_ITERATION
            
            # --- 【核心修改 3】在每个阶段开始时，为所有策略准备一份共同的预热数据子集 ---
            ...
            for strategy in STRATEGIES_TO_RUN:
                ...
                if len(available_df) == 0: logger.warning("候选池已空，跳过此策略。"); continue
                ...
                if strategy == "random": pass
                    ...
                    scores_df = fixed_less_scores_df[fixed_less_scores_df['original_index'].isin(available_indices)]; scores_df['final_score'] = scores_df['fixed_less_score']
                elif strategy == "dynamic_less":
                    imputed_df = run_imputation(state['mt5_model_path'], available_df); scores_df = get_less_scores(state['llama_model_path'], val_grad_path, imputed_df, phase_dir); scores_df['final_score'] = scores_df['less_score']
                elif strategy == "fixed_less_imputation":
                    imp_scores_df = run_entropy_scoring(state['mt5_model_path'], available_df); scores_df = pd.merge(fixed_less_scores_df, imp_scores_df, on='original_index', how='inner')
                    scores_df['norm_less'] = normalize_scores(scores_df['fixed_less_score']); scores_df['norm_imp'] = normalize_scores(scores_df['imputation_score']); scores_df['final_score'] = 0.5 * scores_df['norm_less'] + 0.5 * scores_df['norm_imp']
                elif strategy == "dynamic_less_imputation":
                    imputed_df = run_imputation(state['mt5_model_path'], available_df); less_scores_df = get_less_scores(state['llama_model_path'], val_grad_path, imputed_df, phase_dir); imp_scores_df = run_entropy_scoring(state['mt5_model_path'], available_df)
                    scores_df = pd.merge(less_scores_df, imp_scores_df, on='original_index', how='inner'); scores_df['norm_less'] = normalize_scores(scores_df['less_score']); scores_df['norm_imp'] = normalize_scores(scores_df['imputation_score']); scores_df['final_score'] = 0.5 * scores_df['norm_less'] + 0.5 * scores_df['norm_imp']
                ...
                # --- 【核心修改 4】使用共同的预热子集构建 Llama 和 mT5 的训练集 ---
                llama_train_df = pd.concat([current_selected_df[['full_text']].rename(columns={'full_text': 'text'}), warmup_subset_for_phase[['full_text']].rename(columns={'full_text': 'text'})])
                ...
                logger.info(f"Llama混合训练集: {len(current_selected_df)}条选择数据 + {len(warmup_subset_for_phase)}条预热数据 = {len(llama_train_df)}条总训练数据。")
                ...
                if "imputation" in strategy or "dynamic_less" in strategy: # dynamic_less也需要更新mT5
                    mt5_train_df = pd.concat([current_selected_df[['masked_text']], warmup_subset_for_phase[['masked_text']]])
                    ...
                    logger.info(f"mT5混合训练集: {len(current_selected_df)}条选择数据 + {len(warmup_subset_for_phase)}条预热数据 = {len(mt5_train_df)}条总训练数据。")
                    new_mt5_path = run_mt5_finetuning(train_file_path=mt5_train_file, output_dir=os.path.join(models_dir, "mt5"), num_samples=len(mt5_train_df))
